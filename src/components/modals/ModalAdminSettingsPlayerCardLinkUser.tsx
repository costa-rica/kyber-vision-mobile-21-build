import React, { useState } from "react";
import {
	StyleSheet,
	Text,
	View,
	Dimensions,
	Alert,
} from "react-native";
import ButtonKvNoDefaultTextOnly from "../buttons/ButtonKvNoDefaultTextOnly";
import { Player } from "../../reducers/team";

interface ModalAdminSettingsPlayerCardLinkUserProps {
	playerObject: Player;
	setIsVisibleLinkUserModal: (visible: boolean) => void;
	setPlayerObject: (player: Player) => void;
}

export default function ModalAdminSettingsPlayerCardLinkUser({
	playerObject,
	setIsVisibleLinkUserModal,
	setPlayerObject,
}: ModalAdminSettingsPlayerCardLinkUserProps) {
	return (
		<View style={styles.modalContent}>
			<Text style={styles.txtModalTitle}>Link User Account</Text>
			<Text style={styles.txtModalMessage}>
				Feature coming soon - Link {playerObject.firstName} {playerObject.lastName} to a user account
			</Text>
			<ButtonKvNoDefaultTextOnly
				onPress={() => {
					Alert.alert("Coming Soon", "This feature is not yet implemented");
					setIsVisibleLinkUserModal(false);
				}}
				styleView={styles.btnClose}
				styleText={styles.txtBtnClose}
			>
				Close
			</ButtonKvNoDefaultTextOnly>
		</View>
	);
}

const styles = StyleSheet.create({
	modalContent: {
		width: Dimensions.get("window").width * 0.8,
		height: Dimensions.get("window").height * 0.3,
		borderRadius: 20,
		alignItems: "center",
		justifyContent: "center",
		padding: 20,
		backgroundColor: "#D9CDD9",
		gap: 15,
	},
	txtModalTitle: {
		fontSize: 18,
		fontWeight: "bold",
		textAlign: "center",
		color: "#806181",
	},
	txtModalMessage: {
		fontSize: 14,
		textAlign: "center",
		color: "#333",
	},
	btnClose: {
		justifyContent: "center",
		alignItems: "center",
		width: 100,
		borderRadius: 20,
		backgroundColor: "#E8E8E8",
		borderColor: "#806181",
		borderWidth: 2,
		padding: 10,
	},
	txtBtnClose: {
		fontSize: 16,
		color: "#806181",
	},
});